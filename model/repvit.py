...
#                 # === KEMBALI KE DEFAULT (SqueezeExcite) ===
...
class RepViTBlock(nn.Module):
    def __init__(self, inp, hidden_dim, oup, kernel_size, stride, use_se, use_hs):
        ...
        if stride == 2:
            self.token_mixer = nn.Sequential(
                Conv2d_BN(inp, inp, kernel_size, stride, (kernel_size - 1) // 2, groups=inp),
                # === GANTI SqueezeExcite DENGAN CBAM ===
                # Ratio 4 digunakan agar mirip dengan SE 0.25 (1/0.25 = 4)
                CBAM(inp, ratio=4) if use_se else nn.Identity(),
                # =======================================
                Conv2d_BN(inp, oup, ks=1, stride=1, pad=0)
            )
            self.channel_mixer = Residual(nn.Sequential(
...
def repvit_m2_3(pretrained=False, num_classes=1000, distillation=False, **kwargs):
    ...
    Arguments:
        ...
        distillation (bool): enable distillation mode
        kwargs: extra arguments (untuk menangkap pretrained_cfg dari timm)
    """
    cfgs = [
        # k, t, c, SE, HS, s 
        [3, 2, 80, 1, 0, 1],
